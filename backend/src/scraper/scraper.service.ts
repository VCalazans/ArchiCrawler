...
export class ScraperService {
          ...
          switch (field.action) {
            ...
            case 'upload':
              if (field.value && Array.isArray(field.value)) {
                await engine.uploadFiles(field.selector, field.value as string[]);
              }
              break;
          }
    ...
    try {
      ...
      const result = await retryOperation(async () => {
        ...
        if (request.interceptPatterns && request.interceptPatterns.length > 0) {
          await engine.interceptRequests(async (interceptedRequest: any) => {
  ...
  async batchScrape(request: any): Promise<any> {
    ...
    try {
      ...
      const timeTaken = Date.now() - startTime;
      this.scraperLogger.logError('BATCH_SCRAPE', 'multiple', error as Error, timeTaken);
      throw handleScrapingError(error, 'batch operation');
    }
  }
  ...
  async advancedExtract(request: AdvancedScrapeRequestDto): Promise<any> {
    return this.advancedScrape(request);
  }

  async formInteraction(request: FormInteractionDto): Promise<any> {
    return this.interactWithForm(request);
  }

  async networkIntercept(request: NetworkInterceptDto): Promise<any> {
